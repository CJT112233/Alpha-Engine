...
- **AI-Powered Mass Balance**: Generates mass balances using LLMs for Type A (Wastewater). Type B/C/D use deterministic calculators only (no AI fallback). Prodeval gas upgrading supports 6 tiers: 400, 800, 1,200 SCFM (standard) and 1,500, 1,800, 2,100 SCFM (bespoke). Max capacity 2,100 SCFM.
- **Output Value Cascade**: Editing output parameters (biogas SCFM, RNG SCFM, feed TPD) triggers deterministic downstream recalculation. Override fields defined in `DesignOverrides` interface (`biogasScfm`, `rngScfm`, `feedTpd`, `totalSolidsPct`, `volatileSolidsPct`). Stage-type-aware mapping uses `matchOutputStreamKeyWithContext()` to distinguish digester vs gas upgrading stages. Cascading output fields show blue background + refresh icon tooltip in the UI. `extractDesignOverrides()` accepts optional stages context for accurate field-to-override mapping.
