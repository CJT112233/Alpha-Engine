...
class FeedstockProperty(TypedDict):
    value: str
    unit: str
    confidence: Literal["high", "medium", "low"]
    provenance: str
    group: Literal["identity", "physical", "biochemical", "contaminants", "extended"]
    displayName: str
    sortOrder: int


class FeedstockProfile(TypedDict):
    name: str
    aliases: list[str]
    category: str
    properties: dict[str, FeedstockProperty]


class EnrichedFeedstockSpec(TypedDict):
    value: str
    unit: str
    source: Literal["user_provided", "estimated_default"]
    confidence: Literal["high", "medium", "low"]
    provenance: str
    group: Literal["identity", "physical", "biochemical", "contaminants", "extended"]
    displayName: str
...
feedstock_group_labels: dict[str, str] = {
...
feedstock_group_order: list[str] = [
    "identity",
    "physical",
    "biochemical",
    "contaminants",
...
FEEDSTOCK_LIBRARY: list[FeedstockProfile] = [
    {
        "name": "Potato Waste",
...
def match_feedstock_type(feedstock_name: str) -> Optional[FeedstockProfile]:
    lower = feedstock_name.lower().strip()
    for profile in FEEDSTOCK_LIBRARY:
...
def enrich_feedstock_specs(
    ...
    if profile:
        for key, prop in profile["properties"].items():
            display_lower = prop["displayName"].lower()
            if display_lower not in param_key_map:
                param_key_map[display_lower] = key

    for param_name, param_data in user_provided_params.items():
        normalized_name = param_name.lower().strip()
        mapped_key = param_key_map.get(normalized_name)

        if not mapped_key:
            best_alias = ""
            for alias, key in param_key_map.items():
        ...
        if mapped_key and mapped_key in specs:
            specs[mapped_key] = {
                **specs[mapped_key],
                ...
                "unit": param_data.get("unit", specs[mapped_key]["unit"]),
